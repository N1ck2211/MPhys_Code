...
value_to_count = 'jet_nchad[0]'
name = 'background_nchad0'
...
def build_graph(df, dataset):
    ...
    f = open((name + '.csv'), 'a')
    score_range = np.arange(0, 50, 2)

    for i in score_range:
        cut_phrase = "score >" + str(i)

        df = df.Filter(cut_phrase)
        hist = df.Histo1D(("jet_nchad", "", *bins_n), "jet_nchad")
        count = hist.Integral()
        f.write(str(count) + ',\n')
        print(i)
        print(count)
    ...
    results.append(df.Histo1D(("jet_nchad", "", *bins_n), "jet_nchad"))
