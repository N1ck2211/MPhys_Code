...
outputDir = "./outputs/treemaker/combined_jets/"
...
class RDFanalysis:
    ...
    def analysers(df):
       ...
       #df = df.Define("sum_jets_p4", "JetConstituentsUtils::sum_tlv_constituents({})".format(jetClusteringHelper.constituents), )
        ...
        df = df.Define("jj_p", "JetConstituentsUtils::get_p({})[0]".format(jetClusteringHelper.constituents), )
        df = df.Define("jj_theta", "JetConstituentsUtils::get_theta({})[0]".format(jetClusteringHelper.constituents), )
        df = df.Define("jj_phi", "JetConstituentsUtils::get_phi({})[0]".format(jetClusteringHelper.constituents), )
        df = df.Define("jj_q", "JetConstituentsUtils::get_charge({})[0]".format(jetClusteringHelper.constituents), )

        df = df.Define("jj_e", "JetConstituentsUtils::get_e({})[0]".format(jetClusteringHelper.constituents), )
        df = df.Define("jj_pt", "JetConstituentsUtils::get_pt({})[0]".format(jetClusteringHelper.constituents), )

        return df

    # Mandatory: output function, please make sure you return the branch-list as a python list
    def output():
        branchList = ["jj_m", "jj_p", "jj_theta", "jj_phi", "jj_q", "jj_e", "jj_pt"]
